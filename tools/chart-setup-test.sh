...
helm install vault vault --values values.yaml --wait
...
helm install mongo mongo --namespace=vault --values values.yaml --wait
...
echo starting app
helm install app app --namespace=vault --values values.yaml --wait
